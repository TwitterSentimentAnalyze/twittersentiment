...
```{r}
df_celebrities <- read_csv("./resources/data_celebrities_filtered.csv")
```
```{r}
df_celebrities <- df_celebrities %>% select(c(created_at, screen_name, text, retweet_count, quote_count, reply_count, followers_count))

df_celebrities
```
```{r}
# Create new columns as placeholders
df_celebrities[, c("joy", "trust", "anticipation", "sadness", "fear", "anger", "surprise", "disgust", "sentiment_score")] <- NA
