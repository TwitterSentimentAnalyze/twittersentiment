...
Also, we want to remove the rows that have NA for all the sentiments - joy, trust, anticipation, sadness, fear, anger, surprise, disgust, positive, negative. Having NA for all 8 sentiments mean that the tweet doesn't contain any information to perform sentiment analysis, so we should remove those rows.

```{r}

df <- df[!apply(is.na(df[,c("joy", "trust", "anticipation", "sadness", "fear", "anger", "surprise", "disgust", "positive", "negative")]), 1, all),]
```
