...
We then combined the three groups into a single dataframe with the group indicator column, and saved in data_combined_filtered.csv.
By doing so, we could access the entire data from a single csv file.

After doing so, we made new columns to the dataframe: "joy", "trust", "anticipation", "sadness", "fear", "anger", "surprise", "disgust", "positive", "negative", "sentiment_score". Each column except for "sentiment_score" would have entries of the number of words in each tweet pertaining to that sentiment. By making these columns, we can see numerically, for instance, what was the dominating sentiment in a tweet or how each group reacted to COVID-19 in a certain period of time. 
