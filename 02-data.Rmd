...
We mainly focused on getting datasets for four different groups of influential people/organizations. We used Twitter API (rtweet) to extract user information. The four groups are politicians, medical associations, news media, and celebrities. For each group, we chose top 10 most-followed twitter accounts, and we attempted to extract 10,000 most recent tweets from each user with time range 2020-03-12 23:00:56 ~ 2021-11-28 17:30:42. For the users that have less than 10,000 tweets in that time period, all their tweets are extracted.

Each of us contributed to collecting dataset. Alex ~~, Elin ~~, Kevin found top 100 most influencial medical twitter accounts. (Ref: https://emedcert.com/blog/top-healthcare-twitter-accounts-to-follow)

The main hardship we faced was that there was no API that provides most-followed twitter accounts for specific group. We had to manually look for twitter accounts in politics, media, medical field, and celebrities and list out the top 10 most-followed accounts for each group. For medical associations, we had to narrow down top 100 most followed medical twitter accounts by number of followers provided by get_timelines function from rtweet. 
Also, there are numerous unnecessary or insignificant features in the dataset collected, so we performed data cleaning in the following chapter (3 Data Transformation)
